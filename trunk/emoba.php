...
Author URI: http://kirknet.org/wpplugins
*/
...
function emoba_readable_mail($email="", $name="(Hover)" ) {
  ...
  $email = str_replace('@',
        '</span><span class="emoba-symbol">' . SEP_AT . '</span><span class="emoba-em">',
         $email);
  $return = '<span class="emoba-pop">' . $name . '<span>&nbsp;&nbsp;<span class="emoba-em">';
  $return .= $email . '</span>&nbsp;&nbsp;</span></span>';
  return $return;
}
...
****/
function emoba_addJScript($email, $name, $id) {
  $link   = emoba_hexify_mailto($email);
  $emoba_js = "
<script type=\"text/javascript\">
  var mailtostring = 'mailto:';
  var mailNode = document.getElementById('emoba-$id');
...
// First, convert full  <a href="mailto:A@B.C >Name</a>  links
  ...
  foreach ( $matches as $match ) {
    ...
    $the_name = $match[name];
    $r = rand(1000, 9999);
    $repaddr = "<span id=\"emoba-$r\">";
    $repaddr .= emoba_readable_mail($em_email, $the_name) . "</span>\n";
    $repaddr .= emoba_addJScript($em_email, $the_name, $r);
    $repaddrs[] = $repaddr;
    $targets[] = $match[0];
  }
...
// Finally, convert any remaining addresses A@B.C to link <a href="mailto:A@B.C >A-B-C</a>
  preg_match_all('/'.$addr_pattern.'/i', $content, $matches, PREG_SET_ORDER);
  foreach ( $matches as $match ) {
    $em_email = $match[email];
    $r = rand(1000, 9999);
    $pretty_email = emoba_glyph_email($em_email);
    $dashed_email = emoba_dash_email($em_email);
    $repaddr = "<span id=\"emoba-$r\">";
    $repaddr .= $pretty_email . "</span>\n";
    $repaddr .= emoba_addJScript($em_email, $dashed_email, $r);
    $repaddrs[] = $repaddr;
    $targets[] = $match[0];
  }
  $content = str_replace($targets, $repaddrs, $content);
...
add_filter('comment_text', 'emoba_replace');
